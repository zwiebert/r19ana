...
#define OLD_FORMULAS 0
 ...
 public:
  enum idx_t : uint8_t {
    ...
    idle_regulation,        //  #12
    flags1,  // static 0x88  (bit-5: Closed Loop Readiness (Condition Correction
             // Lambda)!!!)
...
int get_engine_speed_RPM() const {
  uint32_t num = (X(idx_t::engine_speed_lb)) | (X(idx_t::engine_speed_hb) << 8);
...
bool is_throttle_fully_closed() const { return !getbit(X(idx_t::flags0), 3); }
