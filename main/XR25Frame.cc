...
const std::array<uint8_t, 4> XR25Frame::FRAME_START_ID = {0xff, 0x00, 0x10,
                                                          0x77};
...
void XR25Frame::dampbufFill() {
  for (int i = 0; i < FRAME_SIZE; ++i) {
    ...
    if (b == d) continue;
    d = (b > d) ? d + 1 : d - 1;
    if (d >= 0x00 && d <= 0xff) dampbuf[i] = static_cast<uint8_t>(d);
  }
}
...
int XR25Frame::distance() const {
  return (tail < head) ? (tail + BUF_SIZE) - head : tail - head;
}
...
int XR25Frame::find(const std::vector<uint8_t>& data) const {
  for (int i = 0, n = (distance() - (int)data.size()); n > 0; --n, ++i) {
    ...
    for (size_t k = 0; match && k < data.size(); ++k) {
      if (getByteByIndex(k + i) != data[k]) match = false;
    }
    if (match) return i;
  }
  return -1;
...
bool XR25Frame::adjustHead() {
  ...
  while (distance() > FRAME_SIZE) {
    int pos = find(
        std::vector<uint8_t>(FRAME_START_ID.begin(), FRAME_START_ID.end()));
    ...
    if (pos == 0) {
      ...
      head = (last_good_head < 0) ? ((tail + BUF_SIZE - 4) % BUF_SIZE)
                                  : last_good_head;
      break;
    } else {
  ...
  if (last_good_head >= 0) head = last_good_head;
...
bool XR25Frame::add(uint8_t b) {
  ...
  if (m_idx >= FRAME_SIZE) return false;
